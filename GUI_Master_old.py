...
#
lbl = tk.Label(root, text="Insect Detection", font=('times', 35,' bold '), width=55,height=1,bg="#008080",fg="white")
lbl.place(x=0, y=10)
...
#    global fn
    ...
    # y_true = int(os.path.basename(folder_path))-1
    ...
    if fn!="":
        ...
        # accuracy = tf.reduce_mean(tf.cast(tf.equal(yoga, y_true), tf.float32))
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='C:/python code/insect detection', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
...
exit = tk.Button(frame_alpr, text="Exit", command=window, width=15, height=1, font=('times', 15, ' bold '),bg="gray",fg="white")
